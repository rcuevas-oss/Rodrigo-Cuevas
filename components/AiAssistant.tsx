...
export const AiAssistant: React.FC = () => {
  ...
  return (
    <section id="evaluador-ia" className="relative w-full overflow-hidden py-16 md:py-24">
      {/* Aura / halo de fondo */}
      <div
        aria-hidden
        className="pointer-events-none absolute inset-0 -z-10"
      >
        <div className="absolute -top-24 left-1/2 h-[520px] w-[520px] -translate-x-1/2 rounded-full bg-[radial-gradient(circle_at_center,rgba(124,58,237,0.22),transparent_60%)] blur-2xl" />
        <div className="absolute top-36 left-1/2 h-[520px] w-[520px] -translate-x-1/2 rounded-full bg-[radial-gradient(circle_at_center,rgba(59,130,246,0.14),transparent_60%)] blur-2xl" />
      ...
      <div className="mx-auto max-w-5xl px-4 relative z-10">
        ...
        <div className="mx-auto mt-10 max-w-3xl">
          <Card className="rounded-2xl border-slate-200 shadow-lg bg-white/80 backdrop-blur-sm">
            <CardContent className="p-5 sm:p-6">
              ...
              {!expanded ? (
                ...
                <div className="mt-5 grid gap-3 animate-in fade-in slide-in-from-bottom-2 duration-300">
                  ...
                  <div className="mt-2 flex items-center gap-2 text-xs text-slate-500 sm:hidden">
                ...
                <div className="mt-5 animate-in fade-in duration-500">
                   <div className="h-[450px] flex flex-col rounded-xl border border-slate-100 bg-slate-50/50">
                        ...
                        {messages.map((msg, idx) => (
                          <div key={idx} className={`flex gap-3 ${msg.role === 'user' ? 'flex-row-reverse' : ''}`}>
                            <div className={`w-8 h-8 rounded-full flex items-center justify-center shrink-0 border shadow-sm ${
                              msg.role === 'user' ? 'bg-slate-900 text-white border-slate-900' : 'bg-white text-violet-600 border-slate-200'
                            }`}>
                      ...
                      <div className="p-3 bg-white border-t border-slate-200 rounded-b-xl flex gap-2">
                        <input
                          className="flex-1 bg-slate-50 border border-slate-200 rounded-lg px-4 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-violet-500/20 transition-all placeholder:text-slate-400"
                        ...
                        <Button
                          variant="ghost"
                          size="icon"
                          className="h-10 w-10 text-slate-400 hover:text-slate-600 hover:bg-slate-100 rounded-lg"
                          onClick={() => {
